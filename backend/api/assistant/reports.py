"""Genera reportes en PDF para el asistente de voz: o bien un tipo de dato
concreto (consumo de hoy, estimado del mes, plan de ahorro -- ver
`report_data.py`), o el resumen de la conversacion (comportamiento historico,
ahora tambien en PDF). Persiste el archivo en disco y una fila en `reportes`
(ver `api.models.Reporte`).

`sugerir_tipos_reporte` es lo nuevo que decide, segun la conversacion
reciente, cuales de esos tipos tiene sentido ofrecerle al usuario para
descargar (en vez de mostrarle siempre los 4)."""
...
from . import openai_client, pdf_renderer, report_data
