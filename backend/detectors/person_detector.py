...
oficial de Ultralytics, variante "l"), entrenado sobre COCO 2017 (clase unica
"person") en el proyecto independiente `ModelosDeteccionComp/Proyecto_RTDETR_COCO`.

El checkpoint usado (`weights/person/RTDETR_COCO_MEJORADO.pt`) es el `best.pt`
de la corrida del notebook `entrenamientos/RTDETR_COCO_MEJORADO.ipynb` (22
epocas, mejor epoca 19), archivada en
`checkpoints/RTDETR_COCO_prev_20260912_161827/weights/best.pt`. OJO: NO es el
`modelosEntrenados/RTDETR_COCO.pth` actual de ese proyecto; ese export lo
sobrescribio despues la corrida de "RTDETR_COCO copy 5". Es un checkpoint
nativo de Ultralytics (dict con "model", "train_args", etc.) con una unica
clase (`{0: "person"}`).
...
Historial: RT-DETR (pesos de julio) -> Faster R-CNN COCO (2026-09-09) ->
RT-DETR MEJORADO (2026-09-13).
"""
    ...
    def __init__(
        ...
        if not weights_path.exists():
            raise FileNotFoundError(
                ...
                "Copialo desde Proyecto_RTDETR_COCO/checkpoints/"
                "RTDETR_COCO_prev_20260912_161827/weights/best.pt (corrida RTDETR_COCO_MEJORADO)."
            )
        ...
        logger.info(
            "PersonDetector listo (clase=person, arquitectura=RT-DETR, pesos=%s, device=%s)",
            weights_path.name, self.device,
        )
